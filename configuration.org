...
**** Custom Installation Directive
Sometimes I want to use a package that isn't available on Melpa, but rather is
just an Elisp file found somewhere on the internet.

#+BEGIN_SRC emacs-lisp :tangle no
(require 'dash)
(let ((ensure-index (cl-position ':ensure use-package-keywords)))
  (setq use-package-keywords
        (-insert-at ensure-index ':ensure-url use-package-keywords)))

(defcustom use-package-ensure-url-location user-emacs-directory
  :type 'string
  :group 'use-package-ensure)

;; (defun use-package-normalize/:ensure-url ()
;;   )
#+END_SRC
...
#+END_SRC
*** csv-mode
CSV is the "standard" format for tabular data. It stands for Comma Separated
Values, and is just the simplest option of rows similar to "1,3,green,James"
with each row separated by a newline. Variants which use null-character or tab
separators instead of commas, and various other options instead of newlines, are
generally also called CSV. These files can be edited in programs such as Excel,
and they provide much more power than this relatively simple mode, but for quick
edits to small files this mode is quite convenient.
#+BEGIN_SRC emacs-lisp
...
Credit: https://gist.github.com/shanecelis/1049d7f86a45ab10fafe"
  ...
  (c-add-style "spruce-csharp-style"      
               '("bsd"                 
                 (indent-tabs-mode . nil) 
                 (c-basic-offset . 4)     
                 (fill-column . 80)))     
  ;; Sets this as the default style for all cc-mode buffers
  (defun spruce/set-c-style-to-spruce-csharp () 
    (c-set-style "spruce-csharp-style"))
  :hook (csharp-mode . spruce/set-c-style-to-spruce-csharp)
